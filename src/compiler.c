...
/**
 * @brief Label is just an integer index.
 */
typedef int Label;
...
/**
  @brief Distributes new labels, ensuring not duplicates.
  */
Label get_new_label(void)
{
...
/**
  @brief Search for already patched instructions and repatch them to new destination.
...
static inline void compile_if_stmt(Compiler *comp, RakChunk *chunk, uint16_t *off, RakError *err)
{
  ...
  if (!rak_is_ok(err)) return;
  patch_pending_jump(comp, chunk, ifElse, chunk->instrs.len);
...
static inline void compile_while_stmt(Compiler *comp, RakChunk *chunk, RakError *err)
{
  ...
  emit_instr(comp, chunk, rak_jump_instr(loop.off), err);
  if (!rak_is_ok(err)) return;
  patch_pending_jump(comp, chunk, whileEnd, chunk->instrs.len);
  while (&rak_stack_get(&comp->patchList, 0) != patchStartAddress)
  {
    rak_stack_pop(&comp->patchList);
  }
...
static inline void compile_expr_cont(Compiler *comp, RakChunk *chunk, uint16_t *off, RakError *err)
{
  ...
  if (!rak_is_ok(err)) return;
  patch_pending_jump(comp, chunk, orEnd, chunk->instrs.len);
  if (off) *off = _off;
}
...
static inline void compile_and_expr_cont(Compiler *comp, RakChunk *chunk, uint16_t *off, RakError *err)
{
  ...
  if (!rak_is_ok(err)) return;
  patch_pending_jump(comp, chunk, andEnd, chunk->instrs.len);
  if (off) *off = _off;
}
